...
Client deliverables and production artifacts are protected by confidentiality obligations; the projects below are intentionally synthetic, concise examples created to communicate my workflow and solution-design approach.
...
| **Vision Insights** · [plugin](https://github.com/fmlin0429712024/linkhealth-dsh-platform/tree/main/plugins/dsh-vision-insights-plugin) | Reads structured events (zone occupancy, retail item add/remove, mask-wearing compliance) from OpenVINO edge apps over HTTP — deterministic facts, LLM narrates only. The mask-compliance model was trained in-house and benchmarked against an adopted baseline before shipping | ✅ running |
